...
tf.app.flags.DEFINE_integer('batch_size_attention_seq2seq', 64, 'batch size') # 배치 크기
...
tf.app.flags.DEFINE_float('teacher_forcing_rate_attention_seq2seq',
                          0.7, 'teacher forcing rate')  # 학습시 디코더 인풋 정답 지원율
tf.app.flags.DEFINE_string('data_path_attention_seq2seq',
...
tf.app.flags.DEFINE_boolean('teacher_forcing_attention_seq2seq',
                            True, 'Use Teacher Forcing')  # 학습시 디코더 인풋 정답 지원 유무
tf.app.flags.DEFINE_boolean('tokenize_as_morph_attention_seq2seq',
                            False, 'set morph tokenize')  # 형태소에 따른 토크나이징 사용 유무
tf.app.flags.DEFINE_boolean(
    'serving_attention_seq2seq', False, 'Use Serving')  # 서빙 기능 지원 여부
tf.app.flags.DEFINE_boolean('loss_mask_attention_seq2seq', False, 'Use loss mask')   # PAD에 대한 마스크를 통한 loss를 제한
