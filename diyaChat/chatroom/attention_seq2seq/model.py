...
def make_lstm_cell(mode, hiddenSize, index):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        cell = tf.contrib.rnn.DropoutWrapper(
            cell, output_keep_prob=DEFINES.dropout_width_attention_seq2seq)
    return cell
...
def Model(features, labels, mode, params):
    ...
    with tf.variable_scope('decoder_scope', reuse=tf.AUTO_REUSE):
        ...
        for i in range(DEFINES.max_sequence_length_attention_seq2seq):
            ...
            if params['attention'] == True:
                ...
                hidden_with_time_axis = tf.manip.tile(
                    hidden_with_time_axis, [1, DEFINES.max_sequence_length_attention_seq2seq, 1])
                # (?, 25, 1)
    ...
    optimizer = tf.train.AdamOptimizer(
        learning_rate=DEFINES.learning_rate_attention_seq2seq)
