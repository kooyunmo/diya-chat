...
tf.app.flags.DEFINE_integer('batch_size_seq2seq', 64, 'batch size') # 배치 크기
tf.app.flags.DEFINE_integer('train_steps_seq2seq',
                            20000, 'train steps')  # 학습 에포크
tf.app.flags.DEFINE_float('dropout_width_seq2seq',
                          0.5, 'dropout width')  # 드롭아웃 크기
tf.app.flags.DEFINE_integer('layer_size_seq2seq', 3,
                            'layer size')  # 멀티 레이어 크기 (multi rnn)
tf.app.flags.DEFINE_integer('hidden_size_seq2seq', 128, 'weights size') # 가중치 크기
tf.app.flags.DEFINE_float('learning_rate_seq2seq', 1e-3, 'learning rate') # 학습률
tf.app.flags.DEFINE_string(
    'data_path_seq2seq', '../data_in/ChatBotData.csv', 'data path')  # 데이터 위치
tf.app.flags.DEFINE_string('vocabulary_path_seq2seq',
                           '../seq2seq/data_out/vocabularyData.voc', 'vocabulary path')  # 사전 위치
tf.app.flags.DEFINE_string('check_point_path_seq2seq',
                           '../seq2seq/data_out/check_point', 'check point path')  # 체크 포인트 위치
tf.app.flags.DEFINE_integer('shuffle_seek_seq2seq',
