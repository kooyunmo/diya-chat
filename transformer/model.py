...
def feed_forward(inputs, num_units):
    ...
    feature_shape = inputs.get_shape()[-1]
    inner_layer = tf.keras.layers.Dense(num_units, activation=tf.nn.relu)(inputs)     # max(0, xW1 + b1)
...
def scaled_dot_product_attention(query, key, value, masked=False):
    ...
    outputs = tf.matmul(query, key) / tf.sqrt(key_dim_size)      # outputs에는 attention map이 저장됨.
    ...
    if masked:
        ...
        paddings = tf.ones_like(masks) * (-2 ** 32 + 1)      # 가장 작은 수로 초기화된 padding matrix (softmax 통과 후에 값이 0이 되도록)
        outputs = tf.where(tf.equal(masks, 0), paddings, outputs)     # 마스킹된 attention map

    attention_map = tf.nn.softmax(outputs)

    return tf.matmul(attention_map, value)       # 유사도가 반영된 context vectors
