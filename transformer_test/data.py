...
def prepro_like_morphlized(data):
    ...
    for seq in tqdm(data):
        ...
        for word in seq.split():
            morph_tokens = morph_analyzer.morphs(word, stem=True)
            origin2morph_dict = {'origin':word, 'morph':morph_tokens}     # ex) {'origin':'너를', 'morph':['너','를']}

            origin2morph_list.append(origin2morph_dict)

        result_data.append(origin2morph_list)
...
def load_vocabulary():
    ...
    if (not (os.path.exists(DEFINES.vocabulary_path))):
        ...
        if (os.path.exists(DEFINES.data_path)):
            ...
            # 공통적인 단어에 대해서는 모두 필요 없으므로 한개로 만들어 주기 위해서
            # set해주고(중복이 제거된 집합 생성) 이것들을 리스트로 만들어 준다.
